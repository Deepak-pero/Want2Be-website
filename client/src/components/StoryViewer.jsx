...
const markGroupViewed = (groupId) => {
    try {
        const current = JSON.parse(localStorage.getItem(VIEWED_STORIES_KEY) || '{}');
...
const StoryViewer = () => {
    ...
    // Set when a swipe or double-tap is detected on touch, so the
    // synthesized click that follows touchend doesn't ALSO fire a
    // next/prev navigation via the zone's onClick handler.
    ...
    const isLastStory = currentStoryIndex === selectedUserStories.length - 1;
    ...
    // ---- Navigation: identical whether triggered by a laptop mouse click,
    // a confirmed mobile tap, or the keyboard ----
    const nextStory = useCallback(() => {
        if (!selectedUserStories.length) return;
        goToNextStory();
    }, [selectedUserStories.length, goToNextStory]);
    ...
    // 👇 FIX: mark each story as viewed as you move through them (previously
    // this only fired once, for the very first story) and also mark the
    // whole group as viewed for the feed's "seen" ring.
    useEffect(() => {
        if (!selectedUserStories.length) return;
        const story = selectedUserStories[currentStoryIndex];
        if (story) {
            viewStory(story.id).catch(console.error);
        }
        markGroupViewed(userId);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [currentStoryIndex, selectedUserStories, userId]);
    ...
    const triggerHeartBurst = (storyId) => {
    ...
    // ---- Touch handling: ONLY detects swipes and double-taps. A plain
    // single tap is deliberately left alone here — it's handled by the
    // native click event on the left/right zone divs below, which fires
    // the same way for both a mobile tap and a laptop mouse click. This
    // is the fix for "can't move from story 1 to story 2". ----
    ...
    const handleTouchEnd = (e) => {
        ...
        const isTap = Math.abs(diffX) < 15 && Math.abs(diffY) < 15 && elapsed < 500;

        if (isTap) {
            const now = Date.now();
            if (now - lastTapRef.current < DOUBLE_TAP_MS) {
                // Double tap -> like, and suppress the click that follows
                // so it doesn't also navigate to the next story.
                suppressNextClickRef.current = true;
                triggerHeartBurst(currentStoryId);
                lastTapRef.current = 0;
            } else {
                lastTapRef.current = now;
                // Single tap: do nothing here — let the zone's onClick
                // (fired right after touchend) handle navigation.
            }
            return;
        }

        // Swipe up on your own story -> "seen by" list
        ...
        // Horizontal swipe -> next/prev, and suppress the trailing click
    ...
    // Shared handler for the left/right zones — fires identically for a
    // laptop mouse click and a confirmed mobile tap.
    ...
    // Desktop double-click to like (mouse equivalent of double-tap)
    ...
    useEffect(() => {
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [selectedUserStories, currentStoryIndex, showViewers, showDeleteConfirm, nextStory, prevStory]);
    ...
    return (
        ...
        >
            ...
            <button
                onClick={closeStoryViewer}
                className="absolute top-3 right-3 sm:top-4 sm:right-4 text-white text-xl sm:text-2xl z-30 hover:text-gray-300 transition-colors p-2"
                style={{ touchAction: 'none' }}
            >
                ✕
            </button>

            <div className="relative w-full max-w-sm sm:max-w-md h-screen max-h-[600px] sm:max-h-[700px] bg-black group">
                {/* Progress bars */}
                ...
                {/* User info with delete/upload button for own story */}
                ...
                {/* 👇 FIX: Left/Right tap zones now use a single shared click
                    handler with no pointerType check, so a laptop mouse
                    click AND a confirmed mobile tap both navigate. */}
                <div
                ...
                {/* Explicit ‹ › arrows — visible on desktop hover, an
                    obvious extra way to move between stories on a laptop */}
                {!isFirstStory && (
                    <button
                        onClick={(e) => {
                            e.stopPropagation();
                        ...
                        className="hidden sm:flex items-center justify-center absolute left-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/40 text-white text-xl z-20 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/60"
                    >
                        ‹
                    </button>
                )}
                {!isLastStory && (
                    <button
                        ...
                        className="hidden sm:flex items-center justify-center absolute right-2 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/40 text-white text-xl z-20 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-black/60"
                    >
                        ›
                    </button>
                )}
