...
const Stories = () => {
    ...
    const handleFileSelected = async (e) => {
        ...
        try {
            ...
            // Open viewer so user sees their story immediately
            if (currentUserId) {
                navigate(`/story/${currentUserId}`);
            }
        } catch (error) {
    ...
    const getRingClass = (storyUserId, hasStory) => {
        ...
        const viewed = viewedGroups[storyUserId];
    ...
    return (
        <div className="w-full  border-b border-gray-200 shadow-sm">
            {/* Hidden file input — reused so every pick works first try */}
            <input
                ref={fileInputRef}
                type="file"
                accept="image/*"
                    ...
                    {stories.map((story) => (
                        ...
                        >
                            <div className="relative">
                                ...
                                {/* Green dot — only when user is actively online */}
                                {story.isOnline && (
