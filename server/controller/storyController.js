...
const isUserOnline = (lastActive) => {
    ...
    return (Date.now() - new Date(lastActive).getTime()) < FIVE_MINUTES;
};

// ✅ Upload buffer to Cloudinary (works with multer memoryStorage)
const uploadToCloudinary = (buffer, folder) => {
    return new Promise((resolve, reject) => {
        const uploadStream = cloudinary.uploader.upload_stream(
            {
...
// ============================================
export const getAllUsersWithStories = async (req, res) => {
    try {
        const twentyFourHoursAgo = new Date(Date.now() - 24 * 60 * 60 * 1000);

        const allUsers = await User.find({})
            .select('name profilePicture lastActive isOnline');
        ...
        const result = allUsers.map(user => {
            ...
            const userStory = storyMap[userId];
            ...
            return {
                ...
                isOnline: online,
                hasStory: !!userStory,
                stories: userStory ? userStory.stories : [],
...
export const getUserStories = async (req, res) => {
    try {
        ...
        const user = await User.findById(userId).select('name profilePicture lastActive');
...
// ============================================
// ✅ UPLOAD STORY (Uses req.file.buffer for multer memoryStorage)
// ============================================
export const uploadStory = async (req, res) => {
    try {
        console.log('📸 Upload story request received');
        ...
        // ✅ Upload to Cloudinary using buffer (from multer memoryStorage)
